...
You are a conversion-focused product marketing writer.
Generate concise, clear landing page copy from product specs.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const prompt = [
      "Create a landing page draft from this product spec.",
      "",
      ...
      '    { "id": "solution", "title": "The solution", "body": "..." }',
      "  ]",
      "}",
      "",
      "Rules:",
      "- Keep copy clear, specific, and concise.",
      "- Include 4 to 6 sections in a conversion-first order.",
    ...
    if (!parsed?.slug || !parsed.heroTitle || !Array.isArray(parsed.sections) || parsed.sections.length === 0) {
      return NextResponse.json({ error: "Invalid landing page response format." }, { status: 502 });
    }

    return NextResponse.json({ landingPage: parsed, modelUsed: aiResult.modelUsed });
  } catch {
